/* ============================================================
   useAIQuery — envia a pergunta à IA e devolve o mini-paper.
   Exemplo de "mutation": NÃO roda ao montar; dispare via submit().
   ============================================================ */

import { useState } from "react";
import type { Paper, QueryRequest } from "../../types";
...
export function useAIQuery() {
  ...
  async function submit(payload: QueryRequest): Promise<Paper> {
    setLoading(true);
    setError(null);
    try {
      const paper = await postQuery(payload);
      setData(paper);
      return paper;
    } catch (err) {
